...
export class ActivationService {
  ...
  async get(userId: bigint): Promise<ActivationView> {
    ...
    const [shops, productView, favorite, pricingPreview, publishTasks, publishedProduct] =
      await Promise.all([
        ...
        this.prisma.auditLog.findFirst({
          where: { userId, action: 'publish.pricing.preview', outcome: 'success' },
          orderBy: { createdAt: 'asc' },
    ...
    const selectedProduct = productView?.resourceId ?? favorite?.sourceProduct.productId1688;
    const productHref = selectedProduct
      ? `/products?id=${encodeURIComponent(selectedProduct)}`
      : '/';
    const selectedAt = productView?.createdAt ?? favorite?.createdAt ?? null;
