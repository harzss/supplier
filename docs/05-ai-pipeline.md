...
| 层           | 实现                               | 命中场景               |
| ------------ | ---------------------------------- | ---------------------- |
| L1：精确缓存 | Redis SET + key=hash(prompt+model) | 完全相同请求           |
| L2：语义缓存 | Milvus + 阈值 0.95                 | 改了几个字的同语义请求 |
| L3：模板缓存 | 预生成爆款类目模板                 | 冷启动时无 AI 调用     |

**预期命中率**：L1 30% + L2 25% + L3 15% = **70%**，等效成本下降 70%。
