...
>
> **当前实现边界（2026-08-15）**：L1 精确缓存已从 Redis 改为 Supabase PostgreSQL `runtime_states`；读取失败按 cache miss 处理，写入失败跳过缓存，不再把状态降级到当前 BFF 进程内存。`d30d302` 已在 45/45 的 staging 通过 `database + runtimeState` readiness、18/18 smoke 与 runtime-state store/read/consume/lease/renew/release 验证；新缓存边界仍须随当前候选重新部署复验。这只验证缓存所依赖的托管状态链路，不代表真实 LLM/图片服务 E2E 已完成。L2 Milvus 语义缓存与 L3 模板体系仍是目标设计，不得按已上线能力计入命中率或成本。
...
| 层           | 实现状态                                                                                            | 命中场景               |
| ------------ | --------------------------------------------------------------------------------------------------- | ---------------------- |
| L1：精确缓存 | staging 基础链路已验证：`runtime_states` + `key=hash(prompt+model)`；故障时跳过缓存且不保留本机副本 | 完全相同请求           |
