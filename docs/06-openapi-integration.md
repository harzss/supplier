...
- 每笔采购在首次创建快照时持久化原 `buyerShopId`；恢复、付款状态与物流轮询只使用该账号。账号停用或切换后不得改绑当前买家、不得用新账号查询或重放原 `outOrderId`，原账号不可用时 fail-closed
...
- 每次采购状态轮询以 `buyerShopId + attemptNo + outOrderId + orderId1688 + status + retryEligible + exceptionStatus + syncRevision` 完整 attempt 身份原子领取写入所有权，并在落库时继续要求销售订单处于允许履约的售后状态；较早发起但较晚完成的请求不能把新 attempt、新售后状态或新包裹覆盖为旧快照
- 下单结果已知时若并发进入退款/售后，保留已证明的 `orderId1688` 并转 `action_required`；创建与反查结果均未知时保留原 `outOrderId` 并同样转 `action_required`。两类情况都禁止自动重试，必须按原 attempt 人工核对，迟到结果不得覆盖 replacement attempt
...
| A-02 | 1688 | 两个已实名、可授权、可下单的受控测试买家账号；主账号具备人工付款授权和测试人员控制的真实收货地址，第二账号用于数据口径与切换隔离验收   | 1688 买家账号持有人            | ⛔ 待外部条件 | 1688 买家账号与应用 OAuth 授权页                                               | 两个脱敏 member ID、账号状态、收货地址归属确认和付款人授权记录                        |
...
| ---- | ---- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| D-P1 | 抖店 | 商家 OAuth、Token 获取与刷新                | `token.create`、`token.refresh`                                                                                    | 待平台控制台核验                                         | 主体持有人 / 🟡 待核验  | 完成 OAuth、正常刷新、瞬时故障 active + 503、明确拒绝 expired、同主体重新授权恢复；只记录对象、时间和结果，不记录 Token 值     |
...
| A-P1 | 1688 | 买家 OAuth、Token 获取与刷新                | OAuth authorize、authorization code、refresh token 流程                                                            | 待平台控制台核验                                         | 主体持有人 / 🟡 待核验  | OAuth 后验证正常刷新、瞬时故障 active + 503、明确拒绝 expired、同 member 重新授权恢复；只保存 member ID、时间和结果            |
...
| OAuth 安全回退页 | `https://supplier-staging-web.chenjie.workers.dev/settings`                                  | `OAUTH_RESULT_REDIRECT_URL`                         | 工程负责人                     | ✅ 已回填稳定 Web origin；上下文回跳仍只接受可信站内目标       |
...
| T-08 | 买家账号切换    | 用买家 A 建立一笔未付款采购并保留原 outOrderId，随后停用 A、授权 B；恢复必须安全停止或继续使用重新授权后的 A，B 侧不得出现同外部单号或新增采购 | 两个 1688 买家账号持有人 + 工程负责人     | ⛔ 待外部条件 | A/B 脱敏 member ID、采购 buyerShopId、两侧订单列表回读、B 侧零新增与审计记录             |
...
5. 只有 readiness 全绿且 4.2 的官方权限证据齐备，才执行 T-04；T-04 对账成功后再执行 T-05～T-08，其中 T-08 必须在继续使用买家 A 前完成重新授权，绝不向买家 B 重放旧采购。
...
| Token 刷新 | 提前 5min 自动刷新；瞬时/未知故障返回 503 并保留 active，只有明确凭证拒绝或主体不一致才回退 OAuth |
| 限流       | 令牌桶 + 平台维度独立                                                                             |
| 重试       | 幂等键 + 指数退避（仅安全方法）                                                                   |
| 错误映射   | 各平台错误码 → 统一枚举                                                                           |
| 日志       | 全量出入参（敏感信息脱敏）                                                                        |
