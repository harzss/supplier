...
R2-02 当前进入“批量下架 + 批量改标题 + 批量改价 + 同步并核验库存”四个候选代码切片：共用最多 100 件物化预览、item 级状态与逐项结果、停止剩余、仅失败项重试、请求幂等、商品 revision 漂移保护和 Redis 商品锁。标题切片只调用抖店原子标题接口，平台写入前持久化 fence，并在请求前复核锁、所有权和 revision；超时、取消、失锁、worker 中断或畸形 2xx 保持不可重放的未知结果，必须通过草稿感知平台回读核验。库存切片固定 1688 逐 SKU 权威目标，并在不确定时确认平台下架隔离迟到旧写入。批量采集、SKU 编辑、上架、换源和清理仍未实现；第 36～38 个 migration、worker 开关和真实抖店行为也尚未在 staging 验收，因此不能把四个切片写成 R2-02 完成或生产可用。

当前候选已通过全仓 837 项测试（Platform SDK 145、BFF 572、Web 51、其余包 69）、54 项运维脚本、lint 2/2、typecheck 14/14、build 9/9、Prisma validate/format、Prettier、差异检查和联网生产依赖 0 已知漏洞。全新 PostgreSQL 17 已验证 38/38 migration、schema diff、价格/库存快照列、批量表 RLS/ACL 和 sequence 权限；Mock 浏览器完成库存 100 → 80 的预览、刷新恢复、执行、平台回读和 390×844 无页面横向溢出验证。BFF、migration、Web 三个非 root Linux 镜像仍属于 R2-01 之前的已验证基线，本次新增代码尚未重建镜像、部署或应用 staging 第 34～38 个 migration。以下 M13～M67 文字是按里程碑当时证据保留的历史账本，不能覆盖本节最新状态。
...
| 批量商品经营           | 🔄 P0 进行中 | 批量下架、改标题、改价与库存同步切片已有应用侧证据；须在 staging 应用第 36～38 个 migration、验证关闭开关无副作用，并用真实测试商品验收状态/标题/价格/库存回读、未知标题核验、停止、部分失败续跑、重试、漂移和重启恢复 |
