...
R2-02 当前进入“批量下架 + 批量改价 + 同步并核验库存”三个候选代码切片：共用最多 100 件物化预览、item 级状态与逐项结果、停止剩余、仅失败项重试、请求幂等、商品 revision 漂移保护和 Redis 商品锁。库存切片固定 1688 逐 SKU 权威目标，执行前后回读平台、只续跑剩余 SKU，并把最终写入绑定到当前源版本；写入中源版本变化、执行权丢失或结果未知时必须确认平台下架，避免迟到旧写入继续可售。采集端在同一事务更新源库存和所有已发布商品目标，draft/offline 也保留目标，转 online 时再排队；发布落库后再次复核源版本。发布恢复和旧库存 worker 同样不得跳过回读；抖店真实在线商品的完整编辑因无法原子保留库存而 fail-closed，须先下架。批量采集、标题/SKU 编辑、上架、换源和清理仍未实现；第 36～38 个 migration、worker 开关和真实抖店行为也尚未在 staging 验收，因此不能把这三个切片写成 R2-02 完成或生产可用。

当前候选已通过全仓 785 项测试（Platform SDK 133、BFF 537、Web 46、其余包 69）、54 项运维脚本、lint 2/2、typecheck 14/14、build 9/9、Prisma validate/format、Prettier、差异检查和联网生产依赖 0 已知漏洞。全新 PostgreSQL 17 已验证 38/38 migration、schema diff、价格/库存快照列、批量表 RLS/ACL 和 sequence 权限；Mock 浏览器完成库存 100 → 80 的预览、刷新恢复、执行、平台回读和 390×844 无页面横向溢出验证。BFF、migration、Web 三个非 root Linux 镜像仍属于 R2-01 之前的已验证基线，本次新增代码尚未重建镜像、部署或应用 staging 第 34～38 个 migration。以下 M13～M67 文字是按里程碑当时证据保留的历史账本，不能覆盖本节最新状态。
...
| 批量商品经营           | 🔄 P0 进行中 | 批量下架、改价与库存同步切片已有应用侧证据；须在 staging 应用第 36～38 个 migration、验证关闭开关无副作用，并用真实测试商品验收状态/价格/库存回读、停止、部分失败续跑、重试、漂移和重启恢复 |
...
- `PRODUCT_BATCH_ENABLED=false` 时批量预览不产生平台副作用、确认执行返回 503；启用后只允许当前已实现的 `offline`、`edit_price` 与 `sync_inventory`，逐项失败重试不能重跑成功项，改价不得按新基价重复计算；库存不得跳过平台回读或覆盖后来到达的源版本，写入结果未知或源版本竞态必须确认下架隔离
