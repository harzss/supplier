...
| 身份与租户隔离         | 🔄 P0 待验收 | 应用侧已验证 JWT、内部用户映射和数据隔离；须用真实 Supabase 项目完成邮件账号与会话生命周期                                                     |
...
| 真实平台类目/属性/资质 | 🔄 P1        | 应用侧目录、Top3、属性与动态资质闭环已完成；须应用 migration 并用真实测试店验收                                                                |
...
1. **真实身份项目验收**：配置 Supabase Auth 邮件模板、Site URL 与 redirect allowlist，完成注册、验证、登录、刷新、退出和停用账号端到端。
2. **目标环境部署演练**：接入生产 PostgreSQL/Redis、HTTPS 域名、Secret Manager 和镜像仓库，实际执行 migration、灰度切流、应用回滚与隔离数据库恢复。
3. **真实平台闭环**：抖店测试店铺 + 1688 采购 OpenAPI，完成真实商品、订单、采购、物流端到端。
4. **审计与告警外部演练**：接入 HTTPS 接收端、Prometheus/Grafana 与集中日志/APM，验证验签、通知、恢复和应急值守。
5. **外部 AI 与合规**：Storage/GPU/类目目录真实联调，完成 AI 标识、隐私协议和平台审核材料。
