...
>
> **当前 staging 边界（2026-08-08）**：2026-08-06 已完成第 34～43 个 migration、43/43、schema/RLS/ACL 与上一 BFF/Web/Gateway 的 18/18 smoke；仓库现新增第 44 个 SKU 编辑和第 45 个 `runtime_states` migration，所以 staging 当前为 **43/45**，两条均未应用。当前 SHA CI、最新真实备份恢复、43→45 `migrate-forward-once` 与无 Redis 的当前候选动态 smoke 尚未完成；`PRODUCT_BATCH_SKU_EDIT_ENABLED=false`。不得用旧 43/43 smoke 宣称当前候选或生产已经可用。
...
- `NODE_ENV=production`
- `AUTH_MODE=supabase`
- `DATABASE_URL`；migration job 另需 `DIRECT_URL`。不设置 `REDIS_URL`
- `ENCRYPTION_KEY`
...
8. staging 使用专用 `migrate-forward-once`；完成 migration 与断言后才启动新 BFF，并要求 `database + runtimeState` readiness 与动态 smoke 全绿后切流。失败时保持停写，只允许前向修复，不并行重跑、不手工修改 `_prisma_migrations`，也不恢复旧 Redis/BFF 写流量。生产数据库以后仍需独立重复备份、恢复、迁移与动态 smoke，不能复用 staging 结果代替。
